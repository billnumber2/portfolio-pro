...
const STORAGE_KEY = "portfolio-pro-local-email-only-v2";
  ...
  function applyTradeToPositions(trade: Trade) {
    ...
    if (trade.side === "BUY") {
      if (existing) {
        ...
        setPositions(positions.map((p) => p.id === existing.id ? { ...p, shares: newShares, avgCost: Number(newAvgCost.toFixed(4)), currentPrice: tradePrice, currency: marketCurrency(trade.market) } : p));
      } else {

...
function PositionsSection({ positionForm, setPositionForm, addPosition, enriched, positions, setPositions, updatePosition, autoFillPositionFormBySymbol, autoFillExistingPositionBySymbol, sectorLookupMessage, setStatus }: any) {
  return <section className="grid xl:grid-cols-3 gap-6"><Card><h2 className="text-xl font-semibold mb-3">新增庫存</h2><div className="grid grid-cols-2 gap-3"><select className="input" value={positionForm.market} onChange={(e) => setPositionForm({ ...positionForm, market: e.target.value, sector: "", name: "" })}><option value="TW">台股</option><option value="US">美股</option></select><input className="input" placeholder="代號" value={positionForm.symbol} onChange={(e) => setPositionForm({ ...positionForm, symbol: e.target.value.trim().toUpperCase() })} onBlur={() => autoFillPositionFormBySymbol(positionForm.symbol, positionForm.market as Market)} /><input className="input" placeholder="名稱" value={positionForm.name} onChange={(e) => setPositionForm({ ...positionForm, name: e.target.value })} /><input className="input" type="number" placeholder="股數" value={positionForm.shares} onChange={(e) => setPositionForm({ ...positionForm, shares: e.target.value })} /><input className="input" type="number" placeholder="平均成本" value={positionForm.avgCost} onChange={(e) => setPositionForm({ ...positionForm, avgCost: e.target.value })} /><input className="input" type="number" placeholder="現價" value={positionForm.currentPrice} onChange={(e) => setPositionForm({ ...positionForm, currentPrice: e.target.value })} /><input className="input" placeholder="備註" value={positionForm.note} onChange={(e) => setPositionForm({ ...positionForm, note: e.target.value })} /></div><button onClick={addPosition} className="btn w-full mt-3"><Plus size={16} />新增持股</button></Card><Card className="xl:col-span-2"><h2 className="text-xl font-semibold mb-4">庫存明細（可直接修改所有欄位）</h2><div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-sm"><thead><tr className="bg-slate-100 text-slate-600"><th className="p-3">市場</th><th className="p-3">代號</th><th className="p-3">名稱</th><th className="p-3 text-right">股數</th><th className="p-3 text-right">平均成本</th><th className="p-3 text-right">現價</th><th className="p-3 text-right">市值</th><th className="p-3 text-right">損益</th><th className="p-3">備註</th><th className="p-3 text-center">操作</th></tr></thead><tbody>{enriched.map((p: any) => <tr key={p.id} className="border-b"><td className="p-2"><select className="input w-20" value={p.market} onChange={(e) => updatePosition(p.id, "market", e.target.value)}><option value="TW">TW</option><option value="US">US</option></select></td><td className="p-2"><input className="input w-24" value={p.symbol} onChange={(e) => updatePosition(p.id, "symbol", e.target.value.trim().toUpperCase())} onBlur={() => autoFillExistingPositionBySymbol(p.id, p.symbol, p.market)} /></td><td className="p-2"><input className="input w-32" value={p.name} onChange={(e) => updatePosition(p.id, "name", e.target.value)} /></td><td className="p-2 text-right"><input className="input w-24 text-right" type="number" value={p.shares} onChange={(e) => updatePosition(p.id, "shares", e.target.value)} /></td><td className="p-2 text-right"><input className="input w-24 text-right" type="number" value={p.avgCost} onChange={(e) => updatePosition(p.id, "avgCost", e.target.value)} />{p.currency === "USD" && <div className="text-xs text-slate-500">{fmt(p.avgCostTwd)}</div>}</td><td className="p-2 text-right"><input className="input w-24 text-right" type="number" value={p.currentPrice} onChange={(e) => updatePosition(p.id, "currentPrice", e.target.value)} /></td><td className="p-2 text-right"><div>{fmt(p.value, p.currency, p.currency === "USD" ? 2 : 0)}</div><div className="text-xs text-slate-500">{fmt(p.valueTwd)}</div></td><td className={`p-2 text-right font-semibold ${p.pnl >= 0 ? "text-red-600" : "text-emerald-600"}`}>{fmt(p.pnl, p.currency, p.currency === "USD" ? 2 : 0)}<div className="text-xs text-slate-500">{fmt(p.pnlTwd)}</div></td><td className="p-2"><input className="input w-40" value={p.note} onChange={(e) => updatePosition(p.id, "note", e.target.value)} /></td><td className="p-2 text-center"><button onClick={() => { setPositions(positions.filter((x: Position) => x.id !== p.id)); setStatus("已從網站刪除。若要同步刪除雲端資料，請按同步到雲端。"); }} className="text-slate-400 hover:text-red-600"><Trash2 size={16} /></button></td></tr>)}</tbody></table></div></Card></section>;
}

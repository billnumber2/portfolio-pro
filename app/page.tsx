...
export default function Page() {
  ...
  const holdingsSummary = useMemo(() => {
    const totalValue = enriched.reduce((sum, p) => sum + Number(p.valueTwd || 0), 0);
    return enriched
      .map((p) => ({
        id: p.id,
        market: p.market,
        ...
        weight: totalValue > 0 ? (Number(p.valueTwd || 0) / totalValue) * 100 : 0,
      }))

...
export default function Page() {
  ...
  async function fetchLatestFxRate(showMessage = false) {
    try {
      const response = await fetch(`/api/yahoo-fx?symbol=USDTWD=X&t=${Date.now()}`, {
        cache: "no-store",
      });
      const data = await response.json();
      const rate = Number(data?.rate ?? data?.price ?? data?.regularMarketPrice ?? 0);

      if (rate > 0) {
        const roundedRate = Number(rate.toFixed(4));
        setFxRate(roundedRate);
        localStorage.setItem(
          STORAGE_KEY,
          JSON.stringify({ positions, trades, dividends, fxRate: roundedRate })
        );

        if (showMessage) {
          setStatus(`已更新 Yahoo Finance USD/TWD 匯率：${roundedRate.toFixed(4)}`);
        }
      ...
      if (showMessage) {
        setStatus("Yahoo Finance 匯率查詢成功，但回傳值不是有效數字。請檢查 /api/yahoo-fx?symbol=USDTWD=X。");
      }
    } catch (error: any) {
      if (showMessage) {
        setStatus(error?.message || "Yahoo Finance 匯率更新失敗。");
      }
    }

    return Number(fxRate || 32.2);
  }
  ...
  async function refreshCurrentPrices(showMessage = true) {
    ...
    try {
      ...
      if (showMessage) setStatus("正在從 Yahoo Finance 更新匯率與現價...");
      const latestFxRate = await fetchLatestFxRate(false);
      ...
      if (showMessage) setStatus(`已更新 USD/TWD 匯率 ${Number(latestFxRate).toFixed(4)}，並更新 ${updatedCount}/${positions.length} 筆庫存現價，市值與損益已自動重新計算。`);
    } catch (error: any) {
  ...
  return (
    <main className="min-h-screen p-4 md:p-8 bg-slate-50">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {tab === "dashboard" && <Dashboard totals={totals} allocation={allocation} fxRate={fxRate} setFxRate={setFxRate} snapshotChartData={snapshotChartData} snapshotsLoading={snapshotsLoading} holdingsSummary={holdingsSummary} />}
...
function Dashboard({ totals, allocation, fxRate, setFxRate, snapshotChartData, snapshotsLoading, holdingsSummary }: any) {
  return (
    <section className="space-y-6">
      <div className="grid md:grid-cols-4 gap-4">
        <Metric
          ...
          sub={`成本 ${fmt(totals.cost)}`}
        />
        <Metric
          icon={<TrendingUp />}
          title="未實現損益"
          value={fmt(totals.unrealized)}
          sub={pct(totals.pnlPct)}
          positive={totals.unrealized >= 0}
        />
        <Metric
          icon={<ReceiptText />}
          title="已實現損益"
          value={fmt(totals.realized)}
          sub="依賣出交易估算"
          positive={totals.realized >= 0}
        />
        <Metric
          icon={<Coins />}
          title="股利收入"
          value={fmt(totals.dividendIncome)}
          sub={`總報酬 ${fmt(totals.totalReturn)}`}
          positive={totals.dividendIncome >= 0}
        />
      </div>
      ...
      <div className="grid xl:grid-cols-3 gap-6">
        <HistoryChart
          ...
          emptyText="尚無每日快照資料。可先手動開啟 /api/cron/daily-snapshot 產生今日資料。"
        />
        ...
        <HistoryChart
          ...
          color="#dc2626"
          loading={snapshotsLoading}
          emptyText="尚無每日快照資料。每日台灣時間06:00後會自動累積。"
        />
      ...
      <HoldingsSummaryCard holdings={holdingsSummary} />
    </section>
  );
}

...
export default function Page() {
  ...
  async function lookupYahooSector(symbol: string) {
    ...
    try {
      ...
      const response = await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(normalizedSymbol)}?modules=assetProfile`, {
      ...
      setSectorLookupMessage("Yahoo Finance 查詢失敗。若瀏覽器阻擋跨網域，下一版可改用 Next.js API Route 查詢。");
      return "";
    }
  }
          ...
          <section className="grid xl:grid-cols-3 gap-6">
            <Card>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <input className="input" placeholder="代號" value={positionForm.symbol} onChange={(e) => { const next = normalizeSymbol(e.target.value, positionForm.market as Market); setPositionForm({ ...positionForm, symbol: next }); if (positionForm.market === "TW") { const info = lookupTwStockInfo(next); if (info) setPositionForm((prev) => ({ ...prev, symbol: next, name: prev.name || info.name, sector: info.sector })); } }} onBlur={() => autoFillPositionFormBySymbol(positionForm.symbol, positionForm.market as Market)} />
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1200px] text-sm">
                  ...
                  <tbody>{enriched.map((p) => <tr key={p.id} className="border-b">
                    ...
                    <td className="p-2"><input className="input w-24" value={p.symbol} onChange={(e) => { const next = normalizeSymbol(e.target.value, p.market); updatePosition(p.id, "symbol", next); if (p.market === "TW") { const info = lookupTwStockInfo(next); if (info) setPositions((prev) => prev.map((item) => item.id === p.id ? { ...item, name: item.name || info.name, sector: info.sector } : item)); } }} onBlur={() => autoFillExistingPositionBySymbol(p.id, p.symbol, p.market)} /></td>

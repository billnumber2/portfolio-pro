...
const STORAGE_KEY = "portfolio-pro-v5-price-update";
const colors = ["#2563eb", "#16a34a", "#f97316", "#9333ea", "#dc2626", "#0891b2"];

// 台股產業快取表。若要恢復完整 1,134 筆 Excel 對照，後續可再改成 import lib/twStockInfo.ts。
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        setPositions(saved.positions || fallbackPositions);
  ...
  async function autoFillPositionFormBySymbol(symbol: string, market: Market) {
    ...
    if (market === "TW") {
      ...
      if (info) {
        ...
        setSectorLookupMessage(`台股對照表查無代號：${normalizedSymbol}`);
      }
      return;
    }
  ...
  async function refreshCurrentPrices(showMessage = true) {
    ...
    try {
      ...
      const response = await fetch("/api/yahoo-prices", {
        ...
        body: JSON.stringify({
          positions: positions.map((p) => ({ id: p.id, market: p.market, symbol: p.symbol })),
        }),
      });
      const data = await response.json();
  ...
  return (
    <main className="min-h-screen p-4 md:p-8 bg-slate-50">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {tab === "dashboard" && <Dashboard totals={totals} monthlyReport={monthlyReport} allocation={allocation} fxRate={fxRate} setFxRate={setFxRate} />}
...
function Dashboard({ totals, monthlyReport, allocation, fxRate, setFxRate }: any) {
  return <section className="space-y-6"><div className="grid md:grid-cols-4 gap-4"><Metric icon={<WalletCards />} title="總市值" value={fmt(totals.value)} sub={`成本 ${fmt(totals.cost)}`} /><Metric icon={<TrendingUp />} title="未實現損益" value={fmt(totals.unrealized)} sub={pct(totals.pnlPct)} positive={totals.unrealized >= 0} /><Metric icon={<ReceiptText />} title="已實現損益" value={fmt(totals.realized)} sub="依賣出交易估算" positive={totals.realized >= 0} /><Metric icon={<Coins />} title="股利收入" value={fmt(totals.dividendIncome)} sub={`總報酬 ${fmt(totals.totalReturn)}`} positive={totals.dividendIncome >= 0} /></div><div className="grid xl:grid-cols-3 gap-6"><Card className="xl:col-span-2"><h2 className="text-xl font-semibold mb-4">月度統計</h2><div className="h-72"><ResponsiveContainer><BarChart data={monthlyReport}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip formatter={(v) => fmt(Number(v))} /><Legend /><Bar dataKey="buy" name="買進" fill="#2563eb" /><Bar dataKey="sell" name="賣出" fill="#16a34a" /><Bar dataKey="dividends" name="股利" fill="#f97316" /></BarChart></ResponsiveContainer></div></Card><Card><h2 className="text-xl font-semibold mb-4">市場配置</h2><div className="h-72"><ResponsiveContainer><PieChart><Pie data={allocation} dataKey="value" nameKey="name" innerRadius={55} outerRadius={90}>{allocation.map((entry: any, index: number) => <Cell key={entry.name} fill={colors[index % colors.length]} />)}</Pie><Tooltip formatter={(v) => fmt(Number(v))} /><Legend /></PieChart></ResponsiveContainer></div><label className="text-sm flex items-center gap-2">USD/TWD<input type="number" value={fxRate} onChange={(e) => setFxRate(Number(e.target.value) || 0)} className="input w-28" /></label></Card></div></section>;
}

function PositionsSection({ positionForm, setPositionForm, addPosition, enriched, positions, setPositions, updatePosition, autoFillPositionFormBySymbol, autoFillExistingPositionBySymbol, sectorLookupMessage, setStatus }: any) {
  return <section className="grid xl:grid-cols-3 gap-6"><Card><h2 className="text-xl font-semibold mb-3">新增庫存</h2><div className="grid grid-cols-2 gap-3"><select className="input" value={positionForm.market} onChange={(e) => setPositionForm({ ...positionForm, market: e.target.value, sector: "", name: "" })}><option value="TW">台股</option><option value="US">美股</option></select><input className="input" placeholder="代號" value={positionForm.symbol} onChange={(e) => setPositionForm({ ...positionForm, symbol: e.target.value.trim().toUpperCase() })} onBlur={() => autoFillPositionFormBySymbol(positionForm.symbol, positionForm.market as Market)} /><input className="input" placeholder="名稱" value={positionForm.name} onChange={(e) => setPositionForm({ ...positionForm, name: e.target.value })} /><input className="input" placeholder="產業" value={positionForm.sector} onChange={(e) => setPositionForm({ ...positionForm, sector: e.target.value })} /><input className="input" type="number" placeholder="股數" value={positionForm.shares} onChange={(e) => setPositionForm({ ...positionForm, shares: e.target.value })} /><input className="input" type="number" placeholder="平均成本" value={positionForm.avgCost} onChange={(e) => setPositionForm({ ...positionForm, avgCost: e.target.value })} /><input className="input" type="number" placeholder="現價" value={positionForm.currentPrice} onChange={(e) => setPositionForm({ ...positionForm, currentPrice: e.target.value })} /><input className="input" placeholder="備註" value={positionForm.note} onChange={(e) => setPositionForm({ ...positionForm, note: e.target.value })} /></div>{sectorLookupMessage && <div className="mt-3 rounded-2xl bg-slate-100 p-3 text-sm text-slate-600">{sectorLookupMessage}</div>}<button onClick={addPosition} className="btn w-full mt-3"><Plus size={16} />新增持股</button></Card><Card className="xl:col-span-2"><h2 className="text-xl font-semibold mb-4">庫存明細（可直接修改所有欄位）</h2><div className="overflow-x-auto"><table className="w-full min-w-[1200px] text-sm"><thead><tr className="bg-slate-100 text-slate-600"><th className="p-3">市場</th><th className="p-3">代號</th><th className="p-3">名稱</th><th className="p-3 text-right">股數</th><th className="p-3 text-right">平均成本</th><th className="p-3 text-right">現價</th><th className="p-3">產業</th><th className="p-3 text-right">市值</th><th className="p-3 text-right">損益</th><th className="p-3">備註</th><th className="p-3 text-center">操作</th></tr></thead><tbody>{enriched.map((p: any) => <tr key={p.id} className="border-b"><td className="p-2"><select className="input w-20" value={p.market} onChange={(e) => updatePosition(p.id, "market", e.target.value)}><option value="TW">TW</option><option value="US">US</option></select></td><td className="p-2"><input className="input w-24" value={p.symbol} onChange={(e) => updatePosition(p.id, "symbol", e.target.value.trim().toUpperCase())} onBlur={() => autoFillExistingPositionBySymbol(p.id, p.symbol, p.market)} /></td><td className="p-2"><input className="input w-32" value={p.name} onChange={(e) => updatePosition(p.id, "name", e.target.value)} /></td><td className="p-2 text-right"><input className="input w-24 text-right" type="number" value={p.shares} onChange={(e) => updatePosition(p.id, "shares", e.target.value)} /></td><td className="p-2 text-right"><input className="input w-24 text-right" type="number" value={p.avgCost} onChange={(e) => updatePosition(p.id, "avgCost", e.target.value)} />{p.currency === "USD" && <div className="text-xs text-slate-500">{fmt(p.avgCostTwd)}</div>}</td><td className="p-2 text-right"><input className="input w-24 text-right" type="number" value={p.currentPrice} onChange={(e) => updatePosition(p.id, "currentPrice", e.target.value)} /></td><td className="p-2"><input className="input w-28" value={p.sector} onChange={(e) => updatePosition(p.id, "sector", e.target.value)} /></td><td className="p-2 text-right"><div>{fmt(p.value, p.currency, p.currency === "USD" ? 2 : 0)}</div><div className="text-xs text-slate-500">{fmt(p.valueTwd)}</div></td><td className={`p-2 text-right font-semibold ${p.pnl >= 0 ? "text-red-600" : "text-emerald-600"}`}>{fmt(p.pnl, p.currency, p.currency === "USD" ? 2 : 0)}<div className="text-xs text-slate-500">{fmt(p.pnlTwd)}</div></td><td className="p-2"><input className="input w-40" value={p.note} onChange={(e) => updatePosition(p.id, "note", e.target.value)} /></td><td className="p-2 text-center"><button onClick={() => { setPositions(positions.filter((x: Position) => x.id !== p.id)); setStatus("已從網站刪除。若要同步刪除雲端資料，請按同步到雲端。"); }} className="text-slate-400 hover:text-red-600"><Trash2 size={16} /></button></td></tr>)}</tbody></table></div></Card></section>;
}

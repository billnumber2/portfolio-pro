...
async function lookupByQuoteEndpoint(yahooSymbol: string) {
  const urls = [
    ...
    `https://query2.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(
      yahooSymbol
    )}`,
  ];
  ...
  try {
    ...
    if (!Array.isArray(positions) || positions.length === 0) {
      return NextResponse.json(
        { prices: {}, error: "positions is required" },
        { status: 400 }
      );
    }
    ...
    // 逐筆查詢，速度較慢但可降低 Yahoo Finance 擋請求機率
    ...
    return NextResponse.json(
      {
        prices: {},
        error: error?.message || "Yahoo Finance price update failed",
      },
      { status: 200 }
    );
  }
...
export async function GET(request: Request) {
  ...
  const market =
    normalizeSymbol(searchParams.get("market") || "US") === "TW" ? "TW" : "US";
